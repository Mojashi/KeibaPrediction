...
model = pickle.load(open("models/test2.lgb","rb"))
...
dff.insert(0,"pre", model.predict(races.drop("re1.rank", axis=1)))

def testRace(st,en, diff,sig,modds, verbose=0):
    sum = 0
    kake = 0
    tekic = 0
    ...
    for race_id in range(st,en):
        ...
        if pre[aso[3]] - pre[aso[0]] >= diff and pre[aso[0]] < sig and odds["multi_odds1"][race_id] > modds:

            # for i in range(len(X_test)):
            #     print("pre:%s rank:%s %s" % (pre[i], encoded["re1.rank"][i], encoded["re1.single_return"][i]))
            sum -= 100
            kake += 1
            if df["re1.rank"][pf] <= 3:
...
def objective(trial):
    ret = testRace(st,en, trial.suggest_uniform("diff",0,0.4), trial.suggest_uniform("sig",0,1), trial.suggest_uniform("modds",0,10))
    return ret
def opt():
...
def expe():
    testRace(st,en,0,0,0)
    plt.scatter(dff["re1.rank"].tolist(), dff["pre"].tolist(),s=60, c="pink", alpha=0.2, linewidths=2, edgecolors="red")
    plt.show()
opt()