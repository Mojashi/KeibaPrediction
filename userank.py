...
model = pickle.load(open("models/rankw.lgb","rb"))
...
prebuf = model.predict(races.drop(["re1.rank", "re1.time_index"], axis=1))
dff.insert(0,"pre", prebuf)
...
def testRace(st,en, diff,sig,modds, multi=False, verbose=0):
    ...
    for race_id in range(st,en):
        ...
        if pre[aso[0]] - pre[aso[3 if multi else 1]] >= diff and pre[aso[0]] > sig and odds["multi_odds1" if multi else "single_odds"][race_id] > modds:
...
def objective(trial):
    ret = testRace(st,en, trial.suggest_uniform("diff",0,1), trial.suggest_uniform("sig",0,2), trial.suggest_int("modds",100,1000),multi=True)
    return ret
def opt():
