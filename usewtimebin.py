...
model = pickle.load(open("models/bin.lgb","rb"))
...
def objective(trial):
    ret = testRace(st,en, trial.suggest_uniform("diff",0,0.4), trial.suggest_uniform("sig",0,1), trial.suggest_int("modds",100,1000),multi=False)
    return ret
def opt():
...
def expe():
    testRace(st,en,0,0.2,300.0,multi=False)
